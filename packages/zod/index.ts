...
export const literal = <T extends Literal>(literal: T) =>
...
export const literals = <T extends Literal[]>(
  ...args: T
): UnionType<ToLiteralUnion<T>> => new UnionType(args.map(literal)) as any;
...
export const tuple = <T extends [AnyType, ...AnyType[]] | []>(schemas: T) =>
  new TupleType(schemas);
