...
type AssertEqual<T, K> = [T] extends [K]
  ? [K] extends [T]
    ? true
    : false
  : false;
...
describe("Types test", () => {
  ...
  it("record intersection", () => {
    const schema = z
      .record(z.object({ a: z.string() }))
      .and(z.record(z.object({ b: z.string() })));
    const x: AssertEqual<
      z.Infer<typeof schema>,
  ...
  it("object.partial deep", () => {
    ...
    const x: AssertEqual<
      z.Infer<typeof schema>,
      { a?: string; b?: { c?: number; d?: { e?: number } } }
    > = true;
    x;
  });
  ...
  it("partial object should not double wrap optional fields", () => {
    ...
    const x: AssertEqual<
      typeof schema,
      ObjectType<{ a: OptionalType<StringType> }>
    > = true;
    x;
  ...
  describe("runtime shapes", () => {
    ...
    it("should return the correct shape for a dictionary", () => {
      ...
      const x: AssertEqual<
        ObjectType<{ [z.keySignature]: OptionalType<StringType> }>,
        typeof schema
      > = true;
      x;

      type Schema = z.Infer<typeof schema>;
      const y: AssertEqual<{ [key: string]: string | undefined }, Schema> =
        true;
      y;
    });
    ...
    it("should not double wrap optional types in dictionaries", () => {
      ...
      assert.ok(schema instanceof ObjectType);
      const optionalWrapper: AnyType = (schema as any).objectShape[
        z.keySignature
      ];
    ...
    it("should create a new objectType from the pick of a record", () => {
      ...
      const x: AssertEqual<
        ObjectType<{ a: StringType; b: StringType }>,
        typeof schema
      > = true;
      x;
      assert.ok(schema instanceof ObjectType);
  ...
  describe("Recursive intersection", () => {
    ...
    it("typeof schema conditional inference", () => {
      const x: AssertEqual<
        ...
        ObjectType<{
          ...
          r: ObjectType<{
            [z.keySignature]: ObjectType<{ a: StringType; b: NumberType }>;
          }>;
          a: ArrayType<
            ObjectType<{
              [z.keySignature]: ObjectType<{ a: StringType; b: NumberType }>;
    ...
    it("runtime - should recursivle create new Object Types inside object intersection", () => {
      ...
      //@ts-ignore
      assert.equal(
        inspect(shape.r.objectShape[z.keySignature], true, Infinity),
        inspect(shape.o, true, Infinity),
      );
      assert.ok(shape.a instanceof ArrayType);
      //@ts-ignore
      assert.equal(
        inspect(shape.a.schema, true, Infinity),
        inspect(shape.r, true, Infinity),
      );
    });
  });
});
