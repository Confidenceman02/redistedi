...
const coercionTypeSymbol = Object.getOwnPropertySymbols(
  z.number().coerce(),
).find((x) => x.toString() === "Symbol(coercion)") as any;

const catchError = <T extends (...args: any[]) => any>(
  fn: T,
): ((...args: ArgumentsType<T>[]) => Error) => {
...
describe("Zod Parsing", () => {
  describe("type.onTypeError", () => {
    it("should not need to be defined last to take and keep effect", () => {
      const schema = z
        .string()
        .onTypeError("not a string?!")
        .min(3, "should be three characters at least!");
      ...
      assert.strictEqual(
        minErr.message,
        "should be three characters at least!",
      );
    });
  });
  ...
  describe("type.map parsing", () => {
    ...
    it("should be immutable with regards to root schema", () => {
      ...
      const coercionTypeSymbol: any = Object.getOwnPropertySymbols(num).find(
    ...
    it("should apply predicates", () => {
      const adultSchema = z
        ...
        .withPredicate(
          (value) => value.age > 18,
          "must be over 18 to ride the rollercoaster",
        );
  ...
  describe("String parsing", () => {
    ...
    it("should fail if string does not match pattern and use custom error message", () => {
      const schema = z
        .string()
        .pattern(/^hello/, "value should start with hello");
      const err = catchError(schema.parse.bind(schema))("goodbye world");
      assert.strictEqual(err instanceof z.ValidationError, true);
    ...
    it("should fail if string does not match pattern and use custom error message function", () => {
      const schema = z
        ...
        .pattern(
          /^hello/,
          (value) => `value ${JSON.stringify(value)} did notmatch regexp`,
        );
      const err = catchError(schema.parse.bind(schema))("goodbye world");
      assert.strictEqual(err instanceof z.ValidationError, true);
      assert.strictEqual(
        err.message,
        'value "goodbye world" did notmatch regexp',
      );
    });
    ...
    it("should fail if string length is outside  length the range", () => {
      ...
      assert.strictEqual(
        minErr.message,
        "expected string to have length greater than or equal to 3 but had length 2",
      );
      ...
      assert.strictEqual(
        maxErr.message,
        "expected string to have length less than or equal to 6 but had length 8",
      );
    });
    ...
    it("should fail if string length is greater than max", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected string to have length less than or equal to 6 but had length 11",
      );
    });
    ...
    it("should fail with predicate error message if predicate function returns false", () => {
      const schema = z.string({
        predicate: { func: () => false, errMsg: "custom predicate message" },
      });
      const err = catchError(schema.parse.bind(schema))("hello world");
      assert.strictEqual(err instanceof z.ValidationError, true);
    ...
    it("should fail with predicate error message if predicate function returns false - fluent syntax", () => {
      const schema = z
        .string()
        .withPredicate(() => false, "custom predicate message");
      const err = catchError(schema.parse.bind(schema))("hello world");
      assert.strictEqual(err instanceof z.ValidationError, true);
      assert.strictEqual(err.message, "custom predicate message");
    });
    ...
    it("should fail if not all predicates are met", () => {
      const schema = z.string({
        predicate: [
          {
            func: (value: string) => isNaN(Number(value)),
            errMsg: "value must not be a number",
          },
          {
            func: (value: string) => value.startsWith("hello"),
            errMsg: "value must start with hello",
          },
        ],
      });
    ...
    it('should fail "and" with another type', () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be boolean but got string",
      );
    });
    ...
    it("should fail if value is not within valid strings", () => {
      ...
      assert.strictEqual(
        err.message,
        'expected string to be one of: ["hello","world"]',
      );
    });
  ...
  describe("boolean parsing", () => {
    ...
    it("should throw a ValidationError if not a boolean", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be boolean but got object",
      );
    });
  ...
  describe("number parsing", () => {
    ...
    it("should throw a ValidationError if not a number", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be number but got null",
      );
    });
    ...
    it("should fail if number is below min", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected number to be greater than or equal to 0 but got -1",
      );
    });
    ...
    it("should fail if number is greater than max - fluent syntax", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected number to be less than or equal to 10 but got 20",
      );
    });
    ...
    it("should apply validators with coerced values", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected number to be greater than or equal to 42 but got 5",
      );
    });
  ...
  describe("null parsing", () => {
    ...
    it("should throw a ValidationError if not null", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be null but got number",
      );
    });
  });
  ...
  describe("literal parsing", () => {
    ...
    it("should throw a ValidationError if not the same type", () => {
      ...
      assert.strictEqual(
        err.message,
        `expected value to be literal "123" but got 123`,
      );
    });
    ...
    it("should throw validation error if literal is not the same value", () => {
      ...
      assert.strictEqual(
        err.message,
        `expected value to be literal "123" but got "321"`,
      );
    });
  ...
  describe("date parsing", () => {
    ...
    it("should apply predicates", () => {
      ...
      const schema = z
        .date()
        .withPredicate(
          (value) => value.getTime() > now,
          "expected date to be after than current moment",
        )
        .withPredicate(
          (value) => value.getUTCDay() <= 5 && value.getUTCDay() >= 1,
  ...
  describe("optional and nullable modifiers", () => {
    ...
    it("should not allow undefined when nullable schema", () => {
      const err = catchError(nullableSchema.parse.bind(nullableSchema))(
        undefined,
      );
      assert.strictEqual(err instanceof z.ValidationError, true);
      assert.strictEqual(
    ...
    it("should allow access to schema property", () => {
      ...
      const enumFields = Object.entries(s.shape())
        .filter(
          (f) =>
            f[1] instanceof OptionalType && f[1].schema instanceof EnumType,
        )
        .map((f) => f[0]);
  ...
  describe("object parsing", () => {
    ...
    it("should fail if value provided is null", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be object but got null",
      );
    });
    ...
    it("should fail if value provided is an array", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be object but got array",
      );
    });
    ...
    it("should fail if there are unknown keys", () => {
      ...
      assert.strictEqual(
        err.message,
        'unexpected keys on object: ["key","value"]',
      );
    });
    ...
    it("should allow for using allowUnknownKeys for object from intersection", () => {
      ...
      assert.deepStrictEqual(
        schema.try({ name: "myzod", age: 1, cool: true }),
        { name: "myzod", age: 1 },
      );
    });
    ...
    it("should give meaningful error for nested objects errors", () => {
      ...
      const topLevelError = catchError(schema.parse.bind(schema))({
        person: 5,
      });
      ...
      assert.deepStrictEqual((topLevelError as z.ValidationError).path, [
        "person",
      ]);

      const nestedError = catchError(schema.parse.bind(schema))({
        person: { name: 5 },
      });
      ...
      assert.deepStrictEqual((nestedError as z.ValidationError).path, [
        "person",
        "name",
      ]);
    });
    ...
    it("should give meaningful path error for errors occuring within array", () => {
      ...
      assert.deepStrictEqual((err as z.ValidationError).path, [
        "person",
        "friends",
        1,
        "cars",
    ...
    it("should fail if object does not pass predicate", () => {
      const schema = z.object({ a: z.string(), b: z.string() }).withPredicate(
        ...
        (value) =>
          `expected properties "${value.a}" and "${value.b}" to be equal`,
      );
      const err = catchError(schema.parse.bind(schema))({
        a: "hello",
        b: "world",
      ...
      assert.strictEqual(
        err.message,
        'expected properties "hello" and "world" to be equal',
      );
    });
    ...
    it("should collect errors", () => {
      ...
      assert.deepStrictEqual(Object.keys(err.collectedErrors).sort(), [
        "a",
        "b",
      ]);
    });
    ...
    it("should collect errors with user defined messages", () => {
      ...
      assert.strictEqual(
        err.collectedErrors.name?.message,
        "please enter a valid text field",
      );
      assert.deepStrictEqual(err.collectedErrors?.name?.path, ["name"]);
      assert.strictEqual(
        err.collectedErrors.age?.message,
        "please enter a valid number",
      );
      assert.deepStrictEqual(err.collectedErrors.age?.path, ["age"]);
      assert.strictEqual(
        err.collectedErrors.misc?.message,
        "misc must be provided!",
      );
  ...
  it("should collect deeply nested errors as a tree of errors", () => {
    const deeperSchema = z
      .object({ q: z.number(), r: z.number() })
      .collectErrors();
    const deepSchema = z
      .object({ w: deeperSchema, x: z.string(), y: z.number() })
      .collectErrors();
    const schema = z
      .object({ a: deepSchema, b: deepSchema, c: deepSchema, d: z.string() })
      .collectErrors();
    const err = schema.try({
      a: { w: { q: 3 }, x: 3, y: 4 },
      b: { w: { q: 1, r: "2" }, x: "3", y: "4" },
    ...
    assert.deepStrictEqual(
      Object.keys(err.collectedErrors.a.collectedErrors).sort(),
      ["w", "x"],
    );
    assert.ok(
      err.collectedErrors.a.collectedErrors.w instanceof z.ValidationError,
    );
    assert.strictEqual(
    ...
    assert.deepStrictEqual(
      Object.keys(err.collectedErrors.a.collectedErrors.w.collectedErrors),
      ["r"],
    );
    assert.ok(
      err.collectedErrors.a.collectedErrors.w.collectedErrors.r instanceof
        z.ValidationError,
    );
    assert.strictEqual(
    ...
    assert.ok(
      err.collectedErrors.a.collectedErrors.x instanceof z.ValidationError,
    );
    assert.strictEqual(
      err.collectedErrors.a.collectedErrors.x.message,
      "expected type to be string but got number",
    );
    ...
    assert.deepStrictEqual(
      Object.keys(err.collectedErrors.b.collectedErrors).sort(),
      ["w", "y"],
    );
    assert.ok(
      err.collectedErrors.b.collectedErrors.w instanceof z.ValidationError,
    );
    assert.strictEqual(
    ...
    assert.deepStrictEqual(
      Object.keys(err.collectedErrors.b.collectedErrors.w.collectedErrors),
      ["r"],
    );
    assert.ok(
      err.collectedErrors.b.collectedErrors.w.collectedErrors.r instanceof
        z.ValidationError,
    );
    assert.strictEqual(
    ...
    assert.ok(
      err.collectedErrors.b.collectedErrors.y instanceof z.ValidationError,
    );
    assert.strictEqual(
      err.collectedErrors.b.collectedErrors.y.message,
      "expected type to be number but got string",
    );
    ...
    assert.deepStrictEqual(Object.keys(err.collectedErrors.c.collectedErrors), [
      "w",
    ]);
    assert.ok(
      err.collectedErrors.c.collectedErrors.w instanceof z.ValidationError,
    );
    assert.strictEqual(
      err.collectedErrors.c.collectedErrors.w.message,
      "expected type to be object but got undefined",
    );
  });
  ...
  describe("object utility parsing", () => {
    ...
    it("should fail deep partial if unknown keys included in nested objects", () => {
      ...
      assert.strictEqual(
        err.message,
        'error parsing object at path: "b" - unexpected keys on object: ["f"]',
      );
    });
    ...
    it("should return default value if parsing undefined", () => {
      ...
      assert.deepStrictEqual(schema.parse(undefined), {
        a: "hello",
        b: "world",
      });
    });
  ...
  describe("record parsing", () => {
    ...
    it("should fail if value to be parsed is not a record/object", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected type to be object but got string",
      );
    });
    ...
    it("should fail if a value in object does not match the type", () => {
      ...
      assert.strictEqual(
        err.message,
        'error parsing object at path: "a" - expected type to be boolean but got string',
      );
    });
    ...
    it("should pass for record of partial objects", () => {
      const schema = z.record(
        z.partial(z.object({ a: z.string(), b: z.string() })),
      );
      const ret = schema.parse({
  ...
  describe("array parsing", () => {
    ...
    it("should fail if an array element does not match schema", () => {
      ...
      assert.strictEqual(
        err.message,
        "error at [1] - expected type to be string but got number",
      );
    ...
    it("should fail if array does not have provided length", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected array to have length 2 but got 0",
      );
    });
    ...
    it("should fail if array length is less than min", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected array to have length greater than or equal to 2 but got 0",
      );
    });
    ...
    it("should fail if array length is greater than max", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected array to have length less than or equal to 1 but got 2",
      );
    });
    ...
    it("should fail if elements are not unique", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected array to be unique but found same element at indexes 1 and 2",
      );
    });
    ...
    it("should give meaningful path error for objects", () => {
      ...
      assert.strictEqual(
        err.message,
        "error at [0].key - expected type to be number but got string",
      );
    ...
    it("should fail if predicate is not respected", () => {
      const schema = z
        ...
        .withPredicate(
          (value) => value[0] === 0,
          "expected first element to be 0",
        );
    ...
    it("should support string coercion to arrays with a coerce function and nested coeercion - fluent syntax", () => {
      const schema = z
        ...
        .coerce((v) => v.split(",").map((x) => Number(x)));
      const ret = schema.parse("1,2,3");
      assert.deepStrictEqual(ret, [1, 2, 3]);
    });
    ...
    it("should fail if the coerce function throws", () => {
      ...
      assert.strictEqual(
        err.message,
        "error coercing string value to array - Whoops!",
      );
    });
    ...
    it("should allow access to schema property", () => {
      ...
      const strArrayFields = Object.entries(s.shape())
        .filter(
          (f) => f[1] instanceof ArrayType && f[1].schema instanceof StringType,
        )
        .map((f) => f[0]);
    ...
    it("should fail if unique array does not meet minimum length", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected array to have length greater than or equal to 1 but got 0",
      );
    });
  });
  ...
  describe("union parsing", () => {
    ...
    it("should coerce parent object if element is a union of a coerced type", () => {
      ...
      assert.strictEqual(
        (ret2.birthday as Date).getTime(),
        new Date("1991-07-22").getTime(),
      );
    });
  ...
  describe("intersection parsing", () => {
    ...
    it("should fail if the value contains object keys not in Record<object> intersection", () => {
      ...
      const schema = z.intersection(recordA, recordB);
      const err = catchError(schema.parse.bind(schema))({
        key: { a: 2, b: "string", c: true },
      });
      assert.strictEqual(err instanceof z.ValidationError, true);
      assert.strictEqual(
        err.message,
        'error parsing object at path: "key" - unexpected keys on object: ["c"]',
      );
    });

    it("should parse the intersection of partials objects", () => {
      const schema = z.intersection(
        z.partial(z.object({ a: z.string() })),
        z.partial(z.object({ b: z.number() })),
      );
    ...
    it("should fail if intersect of two tuples is not satisfied", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected tuple length to be 2 but got 1",
      );
    });
    ...
    it("should fail if intersect of two tuples is not satisfied - typeError", () => {
      ...
      assert.strictEqual(
        err.message,
        "error parsing tuple at index 1: expected type to be number but got string",
      );
    });
    ...
    it("should intersect two object arrays", () => {
      const schema = z
        ...
        .and(z.array(z.object({ b: z.string() })));
      const ret = schema.parse([
        { a: "hello", b: "world" },
        { a: "number", b: "42" },
      ]);
    ...
    it("should fail if key is missing from intersect two object arrays", () => {
      ...
      assert.strictEqual(
        err.message,
        "error at [1].b - expected type to be string but got undefined",
      );
    });
    ...
    it("should fail if unknown key in intersect two object arrays", () => {
      ...
      assert.strictEqual(
        err.message,
        'error at [1] - unexpected keys on object: ["c"]',
      );
    });
    ...
    it("should throw on intersection of mapped types", () => {
      assert.throws(
        () => z.intersection(z.string().map(Number), z.number()),
        /mapped types cannot be intersected/,
      );
      assert.throws(
        () => z.string().map(Number).and(z.number()),
        /mapped types cannot be intersected/,
      );
      assert.throws(
        () => z.number().and(z.string().map(Number)),
        /mapped types cannot be intersected/,
      );
    });
  ...
  describe("enum parsing", () => {
    ...
    const notRedColorSchema = schema.withPredicate(
      (value) => value !== "red",
      "cannot be red",
    );
    ...
    it("should fail if not part of enum", () => {
      ...
      assert.strictEqual(
        err.message,
        'error "hot fuzz" not part of enum values',
      );
    });
  ...
  describe("partial parsing", () => {
    ...
    it("should pass with empty object for object unions partial", () => {
      const schema = z.partial(
        z.object({ a: z.number() }).or(z.object({ b: z.string() })),
      );
      assert.deepStrictEqual(schema.parse({}), {});
    });
  });
  ...
  describe("pick parsing", () => {
    ...
    it("should fail if value is missing properties from picked record", () => {
      ...
      assert.strictEqual(
        err instanceof z.ValidationError,
        true,
        "Did not throw ValidationError",
      );
      assert.strictEqual(
        err.message,
        'error parsing object at path: "b" - expected type to be number but got undefined',
    ...
    it("should pick the intersection of a record and an object correctly", () => {
      const schema = z.pick(
        z.object({ a: z.string(), b: z.number() }).and(z.record(z.boolean())),
        ["a", "c"],
      );
    ...
    it("should fail if missing key from pick the intersection of a record and an object - Inverted LR", () => {
      const schema = z.pick(
        z.record(z.boolean()).and(z.object({ a: z.string(), b: z.number() })),
        ["a", "c"],
      );
      const err = catchError(schema.parse.bind(schema))({ a: "hello" });
      assert.ok(err instanceof z.ValidationError);
  ...
  describe("omit parsing", () => {
    ...
    it("should default to keysignature", () => {
      const schema = z.omit(
        z.record(z.string()).and(z.object({ b: z.number() })),
        ["b"],
      );
      const err = catchError(schema.parse.bind(schema))({ b: 123 });
      assert.strictEqual(err instanceof z.ValidationError, true);
  ...
  describe("tuple parsing", () => {
    it("should fail if non array is passed as value", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected tuple value to be type array but got null",
      );
    });
    ...
    it("should fail if tuple does not respect predicate function", () => {
      const schema = z
        ...
        .withPredicate(
          ...
          "expected number to indicate length of string",
        );
  ...
  describe("lazy parsing", () => {
    ...
    it("should work with arrays", () => {
      ...
      const ret = schema.parse({
        name: "horror",
        subCategories: [{ name: "gore", subCategories: [] }],
      });
      assert.deepStrictEqual(ret, {
        name: "horror",
        subCategories: [{ name: "gore", subCategories: [] }],
    ...
    it("should fail with appropriate error message", () => {
      ...
      const err = catchError(schema.parse.bind(schema))({
        ...
        subCategories: [
          {
            name: "Gore",
            subCategories: [{ name: "super gore", subCategories: null }],
          },
        ],
      });
      assert.ok(err instanceof z.ValidationError);
    ...
    it("should coerce values", () => {
      ...
      const ret = schema.parse({
        ...
        friends: [
          { name: "quentin", birthday: date.toISOString(), friends: [] },
        ],
      });
  ...
  describe("bigint parsing", () => {
    ...
    it("should throw if value is less than min", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected bigint to be greater than or equal to 5 but got 3",
      );
    });
    ...
    it("should throw if value is greater than max", () => {
      ...
      assert.strictEqual(
        err.message,
        "expected bigint to be less than or equal to 5 but got 8",
      );
    });
    ...
    it("should fail if predicate is not satisfied", () => {
      const schema = z
        .bigint()
        .withPredicate(
          (int) => int % BigInt(2) === BigInt(0),
          "expected bigint to be even",
        );
      const err = catchError(schema.parse.bind(schema))(1);
      assert.ok(err instanceof z.ValidationError);
