...
type ArrayIntersection<
  A1 extends ArrayType<any>,
  A2 extends ArrayType<any>,
...
type TupleIntersection<
  T1 extends TupleType<any>,
  T2 extends TupleType<any>,
...
export class StringType
  extends Type<string>
  implements WithPredicate<string>, Defaultable<string>
{
...
export class NumberType
  extends Type<number>
  implements WithPredicate<number>, Defaultable<number>
{
...
export class BigIntType
  extends Type<bigint>
  implements WithPredicate<bigint>, Defaultable<bigint>
{
...
export class LiteralType<T extends Literal>
  extends Type<T>
  implements Defaultable<T>
{
...
export class OptionalType<T extends AnyType> extends Type<
  Infer<T> | undefined
> {
...
export class NullableType<T extends AnyType>
  extends Type<Infer<T> | null>
  implements Defaultable<Infer<T> | null>
{
  ...
  constructor(readonly schema: T) {
    super();
    (this as any)[coercionTypeSymbol] = (this.schema as any)[
      coercionTypeSymbol
    ];
    (this as any)[shapekeysSymbol] = (this.schema as any)[shapekeysSymbol];
    (this as any)[allowUnknownSymbol] = (this.schema as any)[
...
export class DateType
  extends Type<Date>
  implements WithPredicate<Date>, Defaultable<Date>
{
...
type RequiredKeys<T extends ObjectShape> = Exclude<
  string & keyof T,
  OptionalKeys<T>
>;
...
export class ObjectType<T extends ObjectShape>
  ...
  implements
    WithPredicate<InferObjectShape<T>>,
    Defaultable<InferObjectShape<T>>
{
  ...
  constructor(
    private readonly objectShape: T,
  ...
  private parseObject(
    ...
    for (const key of this[shapekeysSymbol]) {
      try {
        ...
        if (
          schema instanceof UnknownType &&
          !(value as any).hasOwnProperty(key)
        ) {
          throw (schema as any).typeError(
  ...
  private parseObjectCollect(
    ...
    for (const key of this[shapekeysSymbol]) {
      ...
      const result = (schema as any).try((value as any)[key], {
        suppressPathErrMsg: true,
      });
      if (result instanceof ValidationError) {
        hasError = true;
        errs[key] = this.buildPathError(result, key, {
          suppressPathErrMsg: true,
        });
      }
    }
  ...
  private parseMixRecord(
    ...
    for (const key of new Set(
      ...
      try {
        ((this.objectShape[key] || this[keySignature]) as any).parse(
          (value as any)[key],
          { suppressPathErrMsg: true },
        );
      } catch (err: any) {
  ...
  and<K extends AnyType>(schema: K): IntersectionResult<this, K> {
    if (schema instanceof ObjectType) {
      const keySet = new Set<string>([
        ...(this as any)[shapekeysSymbol],
        ...(schema as any)[shapekeysSymbol],
      ]);
      const intersectShape = Array.from(keySet).reduce<ObjectShape>(
        (acc, key) => {
  ...
  implements WithPredicate<InferTuple<T>>, Defaultable<InferTuple<T>>
{
  ...
  and<K extends AnyType>(
    ...
    if (schema instanceof TupleType) {
      ...
      for (
        let i = 0;
        i < Math.max(this.schemas.length, otherSchemaArray.length);
        i++
      ) {
...
function asUnionType(schema: AnyType): UnionType<any> | null {
  ...
  if (
    schema instanceof IntersectionType &&
    (schema as any)._schema instanceof UnionType
  ) {
...
export class IntersectionType<
  T extends AnyType,
  K extends AnyType,
> extends Type<Flat<Infer<T> & Infer<K>>> {
  private _schema: AnyType | null;

  constructor(
    private readonly left: T,
    private readonly right: K,
  ) {
    super();
...
export class EnumType<T>
  extends Type<ValueOf<T>>
  implements Defaultable<ValueOf<T>>, WithPredicate<ValueOf<T>>
{
  private values: any[];
  ...
  withPredicate(
    fn: (value: ValueOf<T>) => boolean,
    errMsg?: ErrMsg<ValueOf<T>> | undefined,
  ): this {
    return withPredicate(this, { func: fn, errMsg });
  }
}
...
// @ts-ignore
export class LazyType<T extends () => AnyType> extends Type<
  Infer<ReturnType<T>>
> {
