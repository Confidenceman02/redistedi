...
export type MappedType<T> = Type<T>&{
  withPredicate: (fn: Predicate<T>[ 'func' ], errMsg?: ErrMsg<T>) =>
      Type<T>& MappedType<T>;
default:
  (value: T|(() => T)) => Type<T>& MappedType<T>;
};

class MTypeClass<T extends AnyType, K> extends Type<K> implements
    WithPredicate<K>, Defaultable<K> {
  ...
  withPredicate(fn: Predicate<K>[ 'func' ],
                errMsg?: ErrMsg<K>): MTypeClass<T, K> {
...
export class ValidationError extends Error {
  ...
  constructor(message: string, path?: (string|number)[],
    ...
    if (collectedErrors !== undefined) {
      message =
          Object.values(collectedErrors)
              .map(err => `error parsing object at path: "${
                       prettyPrintPath(err?.path || [])}" - ${err?.message}`)
              .join('\n');
    }
    super(message);
    this.path = path;
...
export type Infer<T> =
    T extends AnyType ? (T extends Type<infer K>? K : any) : T;

const allowUnknownSymbol = Symbol('allowUnknown');
const shapekeysSymbol = Symbol('shapeKeys');

type ObjectIntersection<O1 extends ObjectType<any>,
                                   O2 extends ObjectType<any>> =
    O1 extends ObjectType<infer Shape1>
    ? O2 extends ObjectType<infer Shape2>
    ? ObjectType < MergeShapes<Shape1, Shape2> extends infer
T extends ObjectShape ? Flat<T>: never >: never: never;

type ArrayIntersection<A1 extends ArrayType<any>, A2 extends ArrayType<any>> =
    A1 extends ArrayType<infer S1>
    ? A2 extends ArrayType<infer S2>? ArrayType<IntersectionResult<S1, S2>>
                                    : never
    : never;

type TupleIntersection<T1 extends TupleType<any>, T2 extends TupleType<any>> =
    T1 extends TupleType<infer S1>
    ? T2 extends TupleType<infer S2>? TupleType<Join<S1, S2>>: never
    : never;
...
export type IntersectionResult<T extends AnyType, K extends AnyType> =
    ...
    ? K extends ObjectType<any>
                    ? ObjectIntersection<T extends infer X extends
                                             ObjectType<any>? X : never,
                                             K extends infer X extends
                                                 ObjectType<any>? X : never>
                    : IntersectionType<T extends infer X extends
                                           ObjectType<any>? X : never, K>
    : T extends ArrayType<any>
    ? K extends ArrayType<any>? ArrayIntersection<T, K>: IntersectionType<T, K>
    : T extends TupleType<any>
    ? K extends TupleType<any>? TupleIntersection<T, K>: IntersectionType<T, K>
    : T extends MTypeClass<any, any>
                    ? never
                    : K extends MTypeClass<any, any>? never
...
const normalizePredicates = <T>(predicate?: Predicate<T>[ 'func' ]|Predicate<T>|
                                Predicate<T>[]): Predicate<T>[]|null => {
  if (!predicate) {
    return null;
  }
...
const appendPredicate = <T>(predicates: Predicate<T>[]|null|undefined, pred: {
  func: (value: T) => boolean;
...
export type StringOptions = {
  ...
  valid?: string[];
  predicate
  ?: Predicate<string>[ 'func' ] | Predicate<string>| Predicate<string>[];
  default?: string | (() => string);
};

export class StringType extends Type<string> implements WithPredicate<string>,
                                                        Defaultable<string> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
                             ? this.defaultValue()
                             : this.defaultValue): string {
    if (typeof value !== 'string') {
      throw this.typeError('expected type to be string but got ' +
                           typeOf(value));
    }
    if (this.predicates) {
  ...
  min(x: number, errMsg?: ErrMsg<string>): StringType {
    return this.withPredicate(
        ...
        errMsg ||
            ((value: string) =>
                 `expected string to have length greater than or equal to ${
                     x} but had length ${value.length}`));
  }
  max(x: number, errMsg?: ErrMsg<string>): StringType {
    return this.withPredicate(
        (value: string) => value.length <= x,
        errMsg || ((value: string) =>
                       `expected string to have length less than or equal to ${
                           x} but had length ${value.length}`));
  }
  ...
  withPredicate(fn: Predicate<string>[ 'func' ],
                errMsg?: ErrMsg<string>): StringType {
    return withPredicate(this, {func : fn, errMsg});
  }
...
export class BooleanType extends Type<boolean> implements Defaultable<boolean> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
                             ? this.defaultValue()
                             : this.defaultValue): boolean {
    if (typeof value !== 'boolean') {
      throw this.typeError('expected type to be boolean but got ' +
                           typeOf(value));
    }
    return value;
  }
...
export type NumberOptions = {
  ...
  coerce?: boolean;
  predicate
  ?: Predicate<number>[ 'func' ] | Predicate<number>| Predicate<number>[];
  default?: number | (() => number);
};

export class NumberType extends Type<number> implements WithPredicate<number>,
                                                        Defaultable<number> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
    ...
    if (typeof value !== 'number') {
      throw this.typeError('expected type to be number but got ' +
                           typeOf(value));
    }
    if (this.predicates) {
      applyPredicates(this.predicates, value);
  ...
  min(x: number, errMsg?: ErrMsg<number>): NumberType {
    return this.withPredicate(
        value => value >= x,
        errMsg || (value => `expected number to be greater than or equal to ${
                       x} but got ${value}`));
  }
  max(x: number, errMsg?: ErrMsg<number>): NumberType {
    return this.withPredicate(
        value => value <= x,
        errMsg || (value => `expected number to be less than or equal to ${
                       x} but got ${value}`));
  }
  ...
  withPredicate(fn: Predicate<number>[ 'func' ],
                errMsg?: ErrMsg<number>): NumberType {
    return withPredicate(this, {func : fn, errMsg});
  }
...
export type BigIntOptions = {
  ...
  max?: number | bigint;
  predicate
  ?: Predicate<bigint>[ 'func' ] | Predicate<bigint>| Predicate<bigint>[];
  default?: bigint | (() => bigint);
};

export class BigIntType extends Type<bigint> implements WithPredicate<bigint>,
                                                        Defaultable<bigint> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
    ...
    try {
      ...
      throw this.typeError('expected type to be bigint interpretable - ' +
  ...
  min(x: number|bigint, errMsg?: ErrMsg<bigint>): BigIntType {
    return this.withPredicate(
        value => value >= x,
        errMsg || (value => `expected bigint to be greater than or equal to ${
                       x} but got ${value}`));
  }
  max(x: number|bigint, errMsg?: ErrMsg<bigint>): BigIntType {
    return this.withPredicate(
        value => value <= x,
        errMsg || (value => `expected bigint to be less than or equal to ${
                       x} but got ${value}`));
  }
  withPredicate(fn: Predicate<bigint>[ 'func' ],
                errMsg?: ErrMsg<bigint>): BigIntType {
    return withPredicate(this, {func : fn, errMsg});
  }
...
export class UndefinedType extends Type<undefined> {
  parse(value: unknown): undefined {
    if (value !== undefined) {
      throw this.typeError('expected type to be undefined but got ' +
                           typeOf(value));
    }
    return value;
  }
...
export class NullType extends Type<null> implements Defaultable<null> {
  ...
  default(): NullType { return withDefault(this, null); }
}

export type Literal = string|number|boolean|undefined|null;

export class LiteralType<T extends Literal> extends Type<T> implements
    Defaultable<T> {
  private readonly defaultValue?: T;
  constructor(private readonly literal: T) { super(); }
  parse(value: unknown = this.defaultValue): T {
    if (value !== this.literal) {
      const typeofValue =
          typeof value !== 'object' ? JSON.stringify(value) : typeOf(value);
      throw this.typeError(`expected value to be literal ${
          JSON.stringify(this.literal)} but got ${typeofValue}`);
    }
    return value as T;
  }
  and<K extends AnyType>(schema: K): IntersectionType<this, K> {
  ...
  default(): LiteralType<T> { return withDefault(this, this.literal); }
}
...
export class UnknownType extends Type<unknown> implements Defaultable<unknown> {
  ...
  default(value: any|(() => any)) { return withDefault(this, value); }
}
...
export class OptionalType<T extends AnyType> extends Type<Infer<T>|undefined> {
  constructor(readonly schema: T) {
    ...
    (this as any)[allowUnknownSymbol] =
        (this.schema as any)[allowUnknownSymbol];
  }
  ...
  required(): T { return clone(this.schema); }
  and<K extends AnyType>(schema: K): IntersectionType<this, K> {
    return new IntersectionType(this, schema);
  }
}
...
export class NullableType<T extends AnyType> extends
    Type<Infer<T>|null> implements Defaultable<Infer<T>|null> {
  ...
  parse(
      ...
      value: unknown = typeof this.defaultValue === 'function'
                           ? this.defaultValue()
                           : this.defaultValue): Infer<T>|null {
    if (value === null) {
      return null;
    }
...
export class DateType extends Type<Date> implements WithPredicate<Date>,
                                                    Defaultable<Date> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
    ...
    const date = typeof value === 'string' ? this.stringToDate(value)
  ...
  withPredicate(fn: Predicate<Date>[ 'func' ],
                errMsg?: ErrMsg<Date>): DateType {
    return withPredicate(this, {func : fn, errMsg});
  }
...
export type ObjectShape = {
  [key: string]: AnyType;[keySignature]?: AnyType
};

type OptionalKeys<T extends ObjectShape> = {
  [key in keyof T]: undefined extends Infer<T[key]>
                                          ? (key extends symbol ? never : key)
                                          : never;
}[keyof T];

type RequiredKeys<T extends ObjectShape> =
    Exclude<string&keyof T, OptionalKeys<T>>;

type InferKeySignature<T extends ObjectShape> =
...
type Flat<T> =
    T extends {} ? (T extends Date ? T : {[key in keyof T] : T[key]}) : T;

type InferObjectShape<T extends ObjectShape> =
    Flat < Eval < InferKeySignature<T>&
...
export type DeepPartialShape<T extends ObjectShape> = {
  [key in
   keyof T]: T[key] extends ObjectType<infer K>
...
type MergeShapes<T extends ObjectShape, K extends ObjectShape> = {
  ...
  ? key extends keyof K ? IntersectionResult<T[key], K[key]>: T[key]
  : key extends keyof K ? K[key] : never;
};
...
export type ObjectOptions<T extends ObjectShape> = {
  ...
  predicate?: | Predicate<InferObjectShape<T>>[ 'func' ] |
               Predicate<InferObjectShape<T>>| Predicate<InferObjectShape<T>>[];
  default?: InferObjectShape<T>| (() => InferObjectShape<T>);
  collectErrors?: boolean;
};

export class ObjectType<T extends ObjectShape> extends
    Type<InferObjectShape<T>> implements WithPredicate<InferObjectShape<T>>,
                                         Defaultable<InferObjectShape<T>> {
  private readonly predicates: Predicate<InferObjectShape<T>>[]|null;
  private readonly defaultValue
      ?: InferObjectShape<T>|(() => InferObjectShape<T>);
  ...
  private _parse: (value: any, parseOpts: ObjectOptions<any>&
                   PathOptions) => InferObjectShape<T>;
  ...
  constructor(private readonly objectShape: T, opts?: ObjectOptions<T>) {
    ...
    this[coercionTypeSymbol] =
        this.defaultValue !== undefined || this[allowUnknownSymbol] ||
        Object.values(this.objectShape)
            .some(schema => (schema as any)[coercionTypeSymbol]) ||
        !!(this.objectShape[keySignature] &&
           (this.objectShape[keySignature] as any)[coercionTypeSymbol]);
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
                             ? this.defaultValue()
                             : this.defaultValue,
        parseOpts: ObjectOptions<any>&PathOptions = {}): InferObjectShape<T> {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
      throw this.typeError('expected type to be object but got ' +
                           typeOf(value));
    }
    ...
    if (!allowUnknown && !this.objectShape[keySignature]) {
      ...
      if (illegalKeys.length > 0) {
        throw this.typeError('unexpected keys on object: ' +
  ...
  private buildPathError(err: ValidationError, key: string,
                         parseOpts: PathOptions): ValidationError {
    const path = err.path ? [ key, ...err.path ] : [ key ];
    const msg = parseOpts.suppressPathErrMsg
                    ? err.message
                    : `error parsing object at path: "${
                          prettyPrintPath(path)}" - ${err.message}`;
    return new ValidationError(msg, path);
  }

  private selectParser(): (value: any, parseOpts: ObjectOptions<any>&
                           PathOptions) => InferObjectShape<T> {
  ...
  private parseObject(value: Object, parseOpts: ObjectOptions<any>&
                      PathOptions): InferObjectShape<T> {
    for (const key of this[shapekeysSymbol]) {
      try {
        ...
        if (schema instanceof UnknownType &&
          ...
          throw (schema as any)
              .typeError(`expected key "${
                  key}" of unknown type to be present on object`);
        }
  ...
  private parseObjectCollect(value: Object, parseOpts: ObjectOptions<any>&
                             PathOptions): InferObjectShape<T> {
    let hasError = false;
      ...
      if (schema instanceof UnknownType &&
        ...
        errs[key] = this.buildPathError(
            (schema as any)
                .typeError(`expected key "${
                    key}" of unknown type to be present on object`),
            key, {suppressPathErrMsg : true});
        continue;
      }
      const result =
          (schema as any).try((value as any)[key], {suppressPathErrMsg : true});
      if (result instanceof ValidationError) {
        hasError = true;
        errs[key] =
            this.buildPathError(result, key, {suppressPathErrMsg : true});
      }
    }
    if (hasError) {
  ...
  private parseObjectConv(value: Object, parseOpts: ObjectOptions<any>&
                          PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    for (const key of this[shapekeysSymbol]) {
      try {
        ...
        convVal[key] = (schema as any).parse((value as any)[key], {
  ...
  private parseObjectConvCollect(value: Object, parseOpts: ObjectOptions<any>&
                                 PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    const errs: any = {};
    let hasError = false;
  ...
  private parseRecord(value: Object, parseOpts: ObjectOptions<any>&
                      PathOptions): InferObjectShape<T> {
    for (const key in value) {
      try {
        (this[keySignature] as any).parse((value as any)[key], {
          suppressPathErrMsg : true
        });
      } catch (err: any) {
        throw this.buildPathError(err, key, parseOpts);
  ...
  private parseRecordCollect(value: Object, parseOpts: ObjectOptions<any>&
                             PathOptions): InferObjectShape<T> {
    let hasError = false;
    const errs: Record<string, ValidationError> = {};
    for (const key in value) {
      const result = (this[keySignature] as any).try((value as any)[key], {
        suppressPathErrMsg : true
      });
      if (result instanceof ValidationError) {
        hasError = true;
  ...
  private parseRecordConv(value: Object, parseOpts: ObjectOptions<any>&
                          PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    for (const key in value) {
      try {
        convVal[key] = (this[keySignature] as any).parse((value as any)[key], {
          suppressPathErrMsg : true
        });
      } catch (err: any) {
        throw this.buildPathError(err, key, parseOpts);
  ...
  private parseRecordConvCollect(value: Object, parseOpts: ObjectOptions<any>&
                                 PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    const errs: any = {};
    let hasError = false;
  ...
  private parseMixRecord(value: Object, parseOpts: ObjectOptions<any>&
                         PathOptions): InferObjectShape<T> {
    for (const key of new Set(
             Object.keys(value).concat(this[shapekeysSymbol]))) {
      try {
        ((this.objectShape[key] || this[keySignature]) as any)
            .parse((value as any)[key], {suppressPathErrMsg : true});
      } catch (err: any) {
        throw this.buildPathError(err, key, parseOpts);
      }
    }
  ...
  private parseMixRecordCollect(value: Object, parseOpts: ObjectOptions<any>&
                                PathOptions): InferObjectShape<T> {
    let hasError = false;
    const errs: Record<string, ValidationError> = {};
    for (const key of new Set(
             Object.keys(value).concat(this[shapekeysSymbol]))) {
      const result =
          ((this.objectShape[key] || this[keySignature]) as any as any)
              .try((value as any)[key], {
                suppressPathErrMsg : true,
              });
      if (result instanceof ValidationError) {
        hasError = true;
  ...
  private parseMixRecordConv(value: Object,

                             parseOpts: ObjectOptions<any>&
                             PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    for (const key of new Set(
             Object.keys(value).concat(this[shapekeysSymbol]))) {
  ...
  private parseMixRecordConvCollect(value: Object,
                                    parseOpts: ObjectOptions<any>&
                                    PathOptions): InferObjectShape<T> {
    const convVal: any = {};
    const errs: any = {};
  ...
  and<K extends AnyType>(schema: K): IntersectionResult<this, K> {
    if (schema instanceof ObjectType) {
      const keySet = new Set<string>([
        ...(this as any)[shapekeysSymbol], ...(schema as any)[shapekeysSymbol]
      ]);
      const intersectShape =
          Array.from(keySet).reduce<ObjectShape>((acc, key) => {
  ...
  pick<K extends T extends {[keySignature] : AnyType}
       ? string : StringTypes<keyof T>>(
      keys: K[],
      opts?: ObjectOptions<
          Flat<Pick<T, Extract<StringTypes<keyof T>, ToUnion<typeof keys>>>&
               (T extends {[keySignature] : AnyType}
                ? T extends {[keySignature] : infer KeySig}
  ...
  omit<K extends StringTypes<keyof T>>(
      keys: K[], opts?: ObjectOptions<Eval<Omit<T, ToUnion<typeof keys>>>>):
      ObjectType<Flat<Omit<T, ToUnion<typeof keys>>>> {
    const pickedKeys: K[] = ((this as any)[shapekeysSymbol] as K[])
                                .filter((x: K) => !keys.includes(x));
    if (!(this as any)[keySignature]) {
      return this.pick(pickedKeys as any, opts as any) as any;
    }
    return (this.pick(pickedKeys as any, opts as any) as AnyType)
        .and(new ObjectType({[keySignature] : (this as any)[keySignature]}));
  }

  partial<K extends ObjectOptions<Eval<DeepPartialShape<T>>>&{deep : true}>(
      opts: K): ObjectType<Flat<DeepPartialShape<T>>>;
  partial<K extends ObjectOptions<Eval<PartialShape<T>>>&PartialOpts>(opts?: K):
  ...
  shape(): T { return Object.assign({}, this.objectShape); }

  withPredicate(fn: Predicate<InferObjectShape<T>>[ 'func' ],
                errMsg?: ErrMsg<InferObjectShape<T>>): ObjectType<T> {
    return withPredicate(this, {func : fn, errMsg});
  }

  default(value: InferObjectShape<T>|
          (() => InferObjectShape<T>)): ObjectType<T> {
...
export type ArrayOptions<T extends AnyType> = {
  ...
  max?: number;
  unique?: boolean;
  predicate?: Predicate<Infer<T>[]>[ 'func' ] | Predicate<Infer<T>[]>|
               Predicate<Infer<T>[]>[];
  default?: Infer<T>[] | (() => Infer<T>[]);
  coerce?: (value: string) => Infer<T>[];
};

export class ArrayType<T extends AnyType> extends Type<Infer<T>[]> implements
    WithPredicate<Infer<T>[]>, Defaultable<Infer<T>[]> {
  ...
  private readonly _parse: (value: unknown, parseOptions
                            ?: PathOptions&ObjectOptions<any>) => any;
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
                             ...
                             : this.defaultValue,
        parseOptions
        ?: PathOptions&ObjectOptions<any>&{coerced?: boolean}): Infer<T>[] {
    if (typeof value === 'string' && typeof this.coerceFn === 'function' &&
        !parseOptions?.coerced) {
      try {
        return this.parse(this.coerceFn(value),
                          {...parseOptions, coerced : true});
      } catch (e: any) {
        if (e instanceof ValidationError) {
          throw e;
        }
        throw new ValidationError('error coercing string value to array - ' +
                                  e.message);
      }
    }
    if (!Array.isArray(value)) {
  ...
  length(value: number, errMsg?: ErrMsg<Infer<T>[]>): ArrayType<T> {
    return this.withPredicate(arr => arr.length === value,
                              errMsg ||
                                  (arr => `expected array to have length ${
                                       value} but got ${arr.length}`));
  }
  min(value: number, errMsg?: ErrMsg<Infer<T>[]>): ArrayType<T> {
    return this.withPredicate(
        arr => arr.length >= value,
        errMsg ||
            (arr => `expected array to have length greater than or equal to ${
                 value} but got ${arr.length}`));
  }
  max(value: number, errMsg?: ErrMsg<Infer<T>[]>): ArrayType<T> {
    return this.withPredicate(
        arr => arr.length <= value,
        errMsg ||
            (arr => `expected array to have length less than or equal to ${
                 value} but got ${arr.length}`));
  }
  unique(): ArrayType<T> {
  ...
  withPredicate(fn: Predicate<Infer<T>[]>[ 'func' ],
                errMsg?: ErrMsg<Infer<T>[]>): ArrayType<T> {
    return withPredicate(this, {func : fn, errMsg});
  }
...
type IntersecWrapper<A extends any, B extends any> = A extends AnyType
    ? B extends AnyType ? IntersectionResult<A, B>: never
    : never;
...
type TupleOptions<T extends any[]> = {
  predicate?: Predicate<InferTuple<T>>[ 'func' ]|Predicate<InferTuple<T>>|
  Predicate<InferTuple<T>>[];
  default?: InferTuple<T>| (() => InferTuple<T>);
};

export class TupleType<T extends AnyType[]> extends
    Type<InferTuple<T>> implements WithPredicate<InferTuple<T>>,
                                   Defaultable<InferTuple<T>> {
  ...
  parse(value: unknown = typeof this.defaultValue === 'function'
                             ? this.defaultValue()
                             : this.defaultValue): InferTuple<T> {
    if (!Array.isArray(value)) {
      throw this.typeError('expected tuple value to be type array but got ' +
                           typeOf(value));
    }
    if (value.length !== this.schemas.length) {
      throw this.typeError(`expected tuple length to be ${
          this.schemas.length} but got ${value.length}`);
    }
    const convValue: any = (this as any)[coercionTypeSymbol] ? [] : undefined;
  ...
  and<K extends AnyType>(schema: K): K extends TupleType<any>
      ? K extends TupleType<infer Arr>
      ? TupleType<Join<T, Arr>>: never: IntersectionType<this, K> {
    if (schema instanceof TupleType) {
      const otherSchemaArray = (schema as any).schemas;
      const nextSchemasArray: AnyType[] = [];
      for (let i = 0;
           i < Math.max(this.schemas.length, otherSchemaArray.length); i++) {
  ...
  withPredicate(fn: Predicate<InferTuple<T>>[ 'func' ],
                errMsg?: ErrMsg<InferTuple<T>>): TupleType<T> {
    return withPredicate(this, {func : fn, errMsg});
  }
...
type UnionIntersection<U extends UnionType<any>, T extends AnyType> =
    U extends UnionType<infer Schemas>? UnionType<{
  [key in keyof Schemas] : Schemas[key] extends
      AnyType ? IntersectionResult<Schemas[key], T>: Schemas[key];
}>
                                      : never;

export class UnionType<T extends AnyType[]> extends
    Type<InferTupleUnion<T>> implements Defaultable<InferTupleUnion<T>> {
  ...
  parse(
      ...
      value: unknown = typeof this.defaultValue === 'function'
                           ? this.defaultValue()
                           : this.defaultValue): InferTupleUnion<T> {
...
function asUnionType(schema: AnyType): UnionType<any>|null {
  ...
  if (schema instanceof IntersectionType && (schema as any)._schema instanceof
                                                UnionType) {
...
export class IntersectionType<T extends AnyType, K extends AnyType> extends
    Type<Flat<Infer<T>&Infer<K>>> {
  private _schema: AnyType|null;

  constructor(private readonly left: T, private readonly right: K) {
    super();

    this[coercionTypeSymbol] = (this.left as any)[coercionTypeSymbol] &&
    ...
    (this as any)[allowUnknownSymbol] =
        !!((this.left as any)[allowUnknownSymbol] ||
           (this.right as any)[allowUnknownSymbol]);

    if ((this.left as any)[shapekeysSymbol] &&
        (this.right as any)[shapekeysSymbol]) {
      //@ts-ignore
      this[shapekeysSymbol] = Array.from(new Set<string>([
  ...
  parse(value: unknown,
        opts?: PathOptions&ObjectOptions<any>): Flat<Infer<T>&Infer<K>> {
    const allowUnknown =
    ...
    if (!allowUnknown && (this as any)[shapekeysSymbol]) {
      ...
      const invalidKeys =
          Object.keys(value as any)
              .filter((key: string) => !expectedShapeKeys.includes(key));
...
export class EnumType<T> extends Type<ValueOf<T>> implements
    Defaultable<ValueOf<T>>, WithPredicate<ValueOf<T>> {
  private values: any[];
  ...
  coerce(opt: EnumCoerceOptions): EnumType<T> {
    return new EnumType(this.enumeration,
                        {defaultValue : this.defaultValue, coerce : opt});
  }
  enum
  (): T { return this.enumeration; }
  withPredicate(fn: (value: ValueOf<T>) => boolean,
                errMsg?: ErrMsg<ValueOf<T>>|undefined): this {
    return withPredicate(this, {func : fn, errMsg});
  }
}
...
function toPartialSchema(schema: AnyType, opts?: PartialOpts): AnyType {
  ...
  if (schema instanceof IntersectionType) {
    return new IntersectionType(toPartialSchema((schema as any).left, opts),
                                toPartialSchema((schema as any).right, opts));
  }
  if (schema instanceof UnionType) {
    return new UnionType(
        (schema as any)
            .schemas.map((schema: AnyType) => toPartialSchema(schema, opts)));
  }
  if (schema instanceof ArrayType) {
    if (opts?.deep) {
...
export class PartialType<T extends AnyType, K extends PartialOpts> extends Type<
  ...
  parse(value: unknown): K extends {deep : true}
...
// @ts-ignore
export class LazyType<T extends() => AnyType> extends
    Type<Infer<ReturnType<T>>> {
